...
# PID controller with conditional integral anti-windup mechanism
...
class controller:
    ...
    def update(self, desired_velocity, MOTOR):
        ...
        #anti windup mechnism - conditonal integral to prevent integral windup when the controller output is saturated and error pushes the output further into saturation
        saturated = abs(self.raw_power) >= 100
        if (not saturated) or (self.error*self.raw_power<0):
             self.integral += self.error*MOTOR.dt 
